    ...
    <el-dialog title="课程信息" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="90px" size="small">
        ...
        <el-form-item label="老师">
          <el-select clearable v-model="form.teacherId" style="width: 100%">
            <el-option
...
export default {
  ...
  created() {
    this.load();
    console.log("我是user",this.user);
  },
  methods: {
      selectCourse(coureseId){
          this.request.post('/course/studentCourse/'+ coureseId + "/" + this.user.id).then(res=>{
              console.log(res);
              if(res.code === '200'){
    ...
    handleDelete(row) {
      console.log(row);
      this.request.delete("/course/" + row.id).then((res) => {
